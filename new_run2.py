...
#                 CSVout(elapsed, [int(i) for i in ch_unscaled])
                nice_output(elapsed, [int(i) for i in ch_unscaled])
        
        
### END EXAMPLE ###
def CSVout(time, digits):
    a = [time,digits]
    np.append(emptyarray, a, axis = 0)
#############################################################################
...
def nice_output(time, digits):
    ...
    sys.stdout.write(
        ...
        + " ".join(["{: 4f}".format(time)])
        + " ".join(["{: d}".format(i) for i in digits])
        + "\n\033[J\0338" # Restore cursor position etc.

    )
# np.savetxt("AccelerometerNewRun2.csv", a, delimiter=",")
