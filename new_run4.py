...
# sys.stdout = open('AccelerometerNewRun2.csv', 'w')
### START EXAMPLE ###
################################################################################
...
#         sys.stdout.flush()
    ...
    for data_row in itertools.cycle(filter_buffer):
            ...
            ch_buffer = ads2.read_continue(CH_SEQUENCE, data_row)
#             ch_buffer[0] = ads2.read_and_next_is(CH_SEQUENCE[0])

            
#             if elapsed > 0.00003333:
#                 timestamp += 0.00003333

            # Calculate moving average of input samples, subtract offset
#             ch_unscaled = np.average(filter_buffer, axis=0) - CH_OFFSET
        
#                 nice_output(elapsed, ch_unscaled)
#                 CSVout(elapsed, ch_unscaled)
            a = [elapsed, ch_buffer[0]]
            emptyarray.append(a)

        
        
### END EXAMPLE ###
def CSVout(time, digits):
    a = [time, digits]
    emptyarray.append(a)
#############################################################################
# Format nice looking text output:
def nice_output(time, digits):

    sys.stdout.write(
          "\0337" # Store cursor position
        + " ".join(["{: 4f}".format(time)])
        + " ".join(["{: f}".format(i) for i in digits])
...
try:
    ...
    np.savetxt("AccelerometerNewRun4.csv", emptyarray, fmt='%s', delimiter=",")
